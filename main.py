...
def generate(user_race, user_class):  # uses input to create character
    ...
    if user_class == 'Cleric':
        ...
        if x % 3 != 0 and x < 9 and random.random() > 0.5:
            x += 1
        elif (x + 2) < 9 and random.random() > 0.5:
            x += 3
        else:
            if x % 3 != 0 and x > 1 and random.random() > 0.5:
                x -= 1
            elif x > 3 and random.random() > 0.5:
                x -= 3
    ...
    if skill_points > len(skill_list[user_class]):
        skill_points = len(skill_list[user_class]) - 1
