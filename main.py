...
def generate(user_race, user_class): #uses input to create character
	...
	if user_race == 'Halfling':
		...
		ud['Weight'] = 30 + roll_dice('1d1') * (ud['Height']-32) #i decided to use 1d1 instead of removing the * to keep this consistent with the rest of the classes
		ud['dex'] += 2
		ud['str'] -= 2
	...
	if user_class == 'Wizard':
		...
		if random.random() > 0.8:user_feats.append('Spell Mastery')
		if random.random() > 0.6:
			ud['Religion'] = 'Boccob'
			ud['Alignment'] = random.choice(alignment1) + random.choice(alignment2[0:2])
...
if __name__ == '__main__':
	...
	if user_race == 'Half Elf':
		ud['Tribe'] = random.choice(human_tribes)
		ud['Name'] = '{} {}'.format(random.choice(tuple(human_first_names[ud['Tribe']])+elf_first_names), random.choice(tuple(human_first_names[ud['Tribe']])+elf_last_names))
		if random.random() > 0.5: ud['Name'] += ' of the clan ' + ud['Tribe']
		ud['Age'] = random.randint(20, 180)
		ud['Height'] = random.randint(68, 73)
		ud['Weight'] = random.randint(100, 180)
...
if __name__ == '__main__':
	...
	if user_race == 'Half Elf':
		...
		if random.random() > 0.5:
			...
			ud['Name'] = random.choice(elf_first_names)
		if random.random() > 0.5:
			ud['Name'] += random.choice(tuple(human_first_names[ud['Tribe']]))
		else:
			ud['Name'] += random.choice(elf_last_names)
		if random.random() > 0.5:
